...
(commonly using ion chamber).'' -- Peter Almond
\end{quote}
...
Transmission is proportional to e\textsuperscript{(-μL),2}(-nHVL/HVL),or
〖10〗\^{}(-xTVL/TVL), when which becomes more convenient for
calculation.
2\^{}(-n)\textless{}0.05→4.3〖HVL〗\_Cerrobend=4.3〖HVL〗\_lead
ρ\_lead/ρ\_cerroband =4.3×1.3×1/0.83=▭6.7cm Chapter 14 Electron Beam
Therapy
...
\chapter{LDR}\label{ldr}

\url{https://doctorlib.info/oncology/principles-practice-radiation-oncology/22.html}

According to ICRU 38 (1985), the dose rate of intracavity brachytherapy,
defined at the reference point (?), can be categorized a:
\textbf{0.4-2.0 Gy/h};

\section[Isotopes]{\texorpdfstring{Isotopes\footnote{A good reference:
  (\url{https://aapm.org/meetings/amos2/pdf/42-11873-3201-79.pdf})}}{Isotopes}}\label{isotopes}

\begin{longtable}[]{@{}lllll@{}}
\caption{\label{tab:isotopes} Frequently used radioactive
isotopes}\tabularnewline
\toprule
...
(Au-198 and Ir-192), and 600 KeV (Cs-137), respectively.

\section{Air kerma strength}\label{air-kerma-strength}

TG-32 (1987) ``Specification of brachytherapy source strength''

The AAPM recommends the \(\color{Green} {air\ kerma\ strength}\)
(\(S_k\)) for the specification of brachytherapy sources. It is defined
as the air kerma rate at a reference distance (e.g.~1 m) from the source
center along the perpendicular bisector. The air kerma strength is
related to the quantity exposure rate by

\begin{equation}
  S_k = \dot X(d) \cdot  \left( \frac{\bar W}{e}\right) \cdot d^2 =\dot K_\delta \cdot d^2 .
  \label{eq:sk}
\end{equation}
...
\item
  ...
  suitable standard source. The product of \(\dot X(d)\) and
  d\textsuperscript{2} is the
  \(\color{Green} {exposure\ rate\ constant}\) at 1 cm. The
\item
  The term \(\left( \frac{\bar W}{e} \right)\) is the averagy energy to
  create an ion pair. It is equal to \textbf{0.876 cGy/R}.
\end{itemize}

The equation above is a revised definition of air-kerma strength (new
cutoff energy 5 keV); experimentally, an aluminum filter is put in front
of
...
The unit of air kerma strength is U, and \textbf{1 U = 1
\(\mu\)Gym\textsuperscript{2}/hr = 1 cGym\textsuperscript{2}/hr}. The
air kerma strength and \(\color{Green} {apparent\ activity}\) conversion
is 1 U = 0.348, 0.243, 0.486, 0.787, and 0.773 mCi for
\textsuperscript{137}Cs, \textsuperscript{192}Ir,
\textsuperscript{198}Au, \textsuperscript{125}I, and
\textsuperscript{103}Pd, respectively.

\begin{longtable}[]{@{}lllll@{}}
\caption{\label{tab:isotopes} Quantities and units for brachytherapy
...
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.22\columnwidth}\raggedright\strut
Air kerma Strength\strut
\end{minipage} & \begin{minipage}[t]{0.11\columnwidth}\raggedright\strut
S\strut
\end{minipage} & \begin{minipage}[t]{0.20\columnwidth}\raggedright\strut
rad\(\cdot\)cm\textsuperscript{2}/hr\strut
\end{minipage} & \begin{minipage}[t]{0.20\columnwidth}\raggedright\strut
Gym\textsuperscript{2}/s\strut
\end{minipage} & \begin{minipage}[t]{0.13\columnwidth}\raggedright\strut
\(\mu\)Gy\(\cdot\)m\textsuperscript{2}/hr\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.22\columnwidth}\raggedright\strut
Exposure rate constant\strut
\end{minipage} & \begin{minipage}[t]{0.11\columnwidth}\raggedright\strut
\((\Gamma_{\delta})_X\)\strut
\end{minipage} & \begin{minipage}[t]{0.20\columnwidth}\raggedright\strut
R\(\cdot\)cm\textsuperscript{2}/(mCi\(\cdot\)h)\strut
\end{minipage} & \begin{minipage}[t]{0.20\columnwidth}\raggedright\strut
Gy\(\cdot\)m\textsuperscript{2}/(Bq\(\cdot\)s)\strut
\end{minipage} & \begin{minipage}[t]{0.13\columnwidth}\raggedright\strut
-\strut
\end{minipage}\tabularnewline
\bottomrule
...
\tightlist
\item
  r denotes the distance (cm) from the center of the active source to
  the point of interest;
\item
  \(\theta\) is the point of interest relative to the source
  longitudinal axis;
\item
  \(\Lambda\) is the exposure dose rate constan`,
  \(\Lambda = \frac{\Lambda_{MC}+\Lambda_{exp}}{2}\);
\item
  S\textsubscript{k} is the air kerma stregth;
\item
  \(G_L(r,\theta)\) is \(\color{Green} {geometry\ function}\), which is
  equal to 1/r\textsuperscript{2} for point source approximation. It
  \textbf{neglects} scattering and attenuation, and provides an
  effective inverse square-law correction.
\item
  \(g_L(r)\) is \(\color{Green} {radial\ dose\ function}\),
  \(g(r) = \frac{D(r,90^o)}{G(r,\theta_0)}/\frac{D(r_0,90^o}{G(r_0, \theta_0)}\).
  It accounts for the fall-off of dose along the transverse axis as a
  result of attenuation and scattering in the medium, capsule
  filtration, and self-absorption.
\item
  \(F(r,\theta)\) is the \(\color{Green} {anisotropy\ function}\),
  \(F(r,\theta) = \frac{\dot D(r,\theta)}{G(r,\theta)}/\frac{\dot D(r_0,\theta_0)}{G(r_0, \theta_0)}\).
  ...
  absorption in tissue. In TG-43U, typically calculated from Monte
  Carlo.
\end{itemize}

Other TG 43 updates include 1) eliminating apparent activity for
specification of source strength, 2) eliminating the anisotropy constant
in favor of the distance dependent 1-D anisotropy function,
\(\phi_an(r)\), and 3) providing guidance on extrapolating tabulated
TG-43 parameters to longer and shorter distance
...
\textbf{Q3 a) b) c)} A \textsuperscript{137}Cs source is normally used
for consistence check (like linac monthly QA) but not calibration.\\
\textbf{Q4 a)} Like external beam radiotherapy, the inverse square law
is always the biggest factor for dose calculation.\\
\textbf{Q5 a)} Should c) and d) be correct?\\
\textbf{Q6 Initial dose rate} \texttt{c)}\\
...
\textbf{Q8 The Paterson-Parker system} \texttt{c)}\\
\textbf{Q9 The Quimby system} \texttt{a)}\\
\textbf{Q10 The Paris system} \texttt{c)\ d)}
...
\section{Q1 Sources of radiation
exposure}\label{q1-sources-of-radiation-exposure}
...
\section{Q2 Stochastic and deterministic
event}\label{q2-stochastic-and-deterministic-event}
...
\section{Q3 TDS rule}\label{q3-tds-rule}
...
\item
  ...
  \item
    ...
    patients), but HDR is more convenient and accurate.\\
  \end{itemize}
\item
...
\textbf{How high is high?} According to ICRU 38 (1985), the dose rate,
defined at the reference point (1 cm), can be categorized as high if the
dose rate is higher than \textbf{20 cGy/min} or \textbf{12 Gy/hr}. The
modern HDR can deliver at 7 Gy/min. (The first ICRU report was published
in 1927 on internaltion units and standards for x-ray work.)

\(\color{Purple} {\text{Theoretically, HDR has a lower therapeutic ratio than LDR because of the short duration of the treatments. How?}}\)
- Practical ROP chapter ``Intracavitary Brachytherapy''
...
\item
  \href{https://www.ncbi.nlm.nih.gov/pmc/articles/PMC5875818/pdf/ACM2-18-89.pdf}{A
  review of nonstandardized applicators digitization in NucletronTM HDR
  procedures}
\end{itemize}

\begin{quote}
The most common error in administering the HDR brachytherapy procedure
is failure to enter the correct treatment distance.
\end{quote}
...
The \emph{special activity} (SA) is the activity per mass, which can
calculated by: \[SA = \frac{0.693}{T_{1/2}}\times\frac{N_A}{A_w}\]
Therefore, the ration of the S.A. of \textsuperscript{60}Co to that of
\textsuperscript{192}Ir will be \[
\frac{SA_{Co}}{SA_{Ir}} = \frac{74\ days \times 192}{30\ years \times 60} \approx 0.02
\] \#\# Treatment sites
...
GEC-ESTRO target volumes\footnote{Schwarz 2015 AAPM Spring Clinical
  Meeting ``Defining Targets for Brachytherapy''
  \url{https://www.aapm.org/education/vl/vl.asp?id=4077}}

\begin{itemize}
\tightlist
\item
  \emph{Gross tumor volume (diagnosis)} (GTV\textsubscript{D})
  ...
  \end{itemize}
\item
  \emph{Gross tumor volume (brachy)} (GTV\textsubscript{B1},
  ...
  \end{itemize}
\item
  \emph{High risk CTV} (HR CTV\textsubscript{B1}, HR
  ...
  \end{itemize}
\item
  \emph{Intermediate risk CTV} (IR CTV\textsubscript{B1}, IR
  CTV\textsubscript{B2}, \ldots{})
...
\tightlist
\item
  GEC-ESTRO: T2 and plastic applicatior;
\item
  GEC-ESTRO: difference between plastic and titanium
  ...
  \item
    ...
    higher magnetic strength; worse on T2.
  \end{itemize}
\item
  T1 for applicator
\item
...
\textbf{Q2} a)\\
Using Eq. \eqref{eq:sk}, the Source strength in U can be converted from
apparent activity (Ci) by

\begin{equation}
S_k = 10,000\ (mCi) \times 4.69 \left(\frac{{R\cdot cm}^2}{mCi\cdot hr} \right) \times 0.876 \left(\frac{cGy}{R}\right) = \boxed{4.11\times 10^4}\ U \text{ or } \left(\frac{cGy\cdot cm^2}{hr} \right)
\end{equation}
...
Solutions: c a d a b c e c b a b d

\url{https://www.aapm.org/education/sams/Default.asp?v=true\&mid=97\&qid=2264}
B a b c d \emph{d} d c c c

\url{https://www.aapm.org/education/sams/Default.asp?v=true\&mid=127\&qid=2490}

Which imaging modality can localize metal needles to an accuracy of at
least 1mm? \textbf{CT}. MRI. Both CT and MRI. should be None of the
above.

Reference: Wang et al, de Leeuw et al). Localization of the first dwell
position on MRI is within 1-2mm (de Leeuw et al).
...
Methods for verifying applicator placement for interstitial GYN
brachytherapy typically include at least one of the following, EXCEPT:
Clinical examination. CT imaging. MR imaging. \emph{PET imaging}.
Laparoscopy.

Reference: ABS Consensus Guidelines for Interstitial Brachytherapy for
Vaginal Cancer, Beriwal S et al, Brachytherapy 11 (2012) 68-75
...
All of the following are advantages of using an MR system with a higher
magnetic field strength (e.g., 3 T versus 1.5 T) EXCEPT: Higher
signal-to-noise ratio (SNR). \emph{Smaller applicator induced
susceptibility artifacts}. Shorter acquisition times. Improved contrast
in the uterine cervix and vagina.
...
J Radiation Oncology, 80 (3), 947-55 (2011). Dimopoulous et al.,
...
image based adaptive cervix cancer brachytherapy,'' Radiotherapy and
Oncology, 103, 113-22 (2012).

If an applicator has been shown to be MR conditional for a 1.5T MRI,
then it can be safely used in a 3T system without the need for further
testing: True. \emph{False}.
