...
```{r echo=FALSE, warning=FALSE, compton, fig.align='center', out.width='60%',fig.cap= 'CT indicator incorrectly attached to the Leksell G frame. (The image is taken from https://aapm.onlinelibrary.wiley.com/doi/abs/10.1120/jacmp.v17i3.5944)'}
...
```{block, type='FOO'}
During CT or MR simulation, does the frame have to been perfectly leveled?
```
...
```{block, type='FOO'}
During stereotactic localization, you identify the indicator marker on CT or MR axial scans. how does this process tell the system where is origin and where is the isocenter?
```

kladjflkasdjfl;asdjfk;akdfl
